...
public class AddressBookController : ControllerBase
{
    ...
    [HttpPost("")]
    public IActionResult SaveAddress([FromBody] AddressRequestDTO addressModel)
    {
        try
        {
            if (addressModel == null)
            {
                ...
                return BadRequest(new ResponseDTO<string> { Success = false, Message = "Address details must be provided.", Data = null });
            }

            var (userId, role) = GetAuthenticatedUser();
            if (role == "Admin")

                return BadRequest(new ResponseDTO<string> { Success = false, Message = "Admin cannot save personal contacts.", Data = null });
            ...
            if (savedEntity == null)
            {
                ...
                return StatusCode(500, new ResponseDTO<string> { Success = false, Message = "Unable to save address." });
            }

            return Ok( new ResponseDTO<Addresses>
            {
                Success = true,
    ...
    public async Task<IActionResult> GetAddressById(int id)
    {
        try
        {
            ...
            if (entity == null)
                return NotFound(new ResponseDTO<string> { Success = false, Message = "Address not found In DB.", Data = null });
    ...
    public async Task<IActionResult> GetAllAddresses()
    {
        ...
        return Ok(new ResponseDTO<List<Addresses>>
        {
            Success = true,
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> EditAddress(int id, [FromBody] AddressRequestDTO addressModel)
    {
        try
        {
            var (userId, role) = GetAuthenticatedUser();
            ...
            if (result == null)
                return NotFound(new ResponseDTO<string> { Success = false, Message = "Address not found to update.", Data = null });
    ...
    public async Task<IActionResult> DeleteAddress(int id)
    {
        try
        {
            ...
            if (!result)
                return NotFound(new ResponseDTO<string> { Success = false, Message = "Address not found to delete.", Data = null });

            return Ok(new ResponseDTO<string> { Success = true, Message = "Address deleted successfully." });
        }
        catch (Exception ex)
        {
            return StatusCode(500, new ResponseDTO<string> { Success = false, Message = ex.Message });
